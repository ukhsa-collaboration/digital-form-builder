...
1. Extracts the **project id** from the first segment of the request path
   (`extractProjectIdFromPath`), e.g. `/my-form/page-one` → `my-form`.
2. Looks up `server.app.forms[projectId]` to get the form's model, and reads its **group** from
   `form.def.group` (the optional `group` field on the form JSON).
3. If the thrown error's message identifies it as a `ControllerError` or `RenderingError`, it is
   ...
   - `page` view (from `ControllerErrorMetadata.page`, if provided) in the **project** folder,
...
4. Any other Boom error falls back to the pre-existing behaviour:
   - a `403` renders `csrf-protection` (CSRF token failures), passing the project id and form
     name into the view.
   - anything else renders the generic `500` view, passing the form name (or the configured
     `serviceName`) into the view.
5. If a form cannot be resolved for the project id (e.g. the path doesn't map to a known form),
   resolution falls back to the plain `500` view with no project context.

## View lookup order

Given a project id, an optional group, and a view name (either a `page` name or a status code),
...
`my-form` and `group: "my-group"` will look for:

```
views/my-form/404.html
views/my-group/404.html
views/404.html
```
...
views/my-group/example-error.html
views/example-error.html
```

and only fall back to the `500` lookup above if none of the `example-error.html` files exist.

## Adding a project or group-specific error page

Add an `.html` view under a folder matching the project id or `group` value, using the same name
...
├── my-group/
│   └── 404.html             # shared by all forms with group: "my-group"
└── my-form/

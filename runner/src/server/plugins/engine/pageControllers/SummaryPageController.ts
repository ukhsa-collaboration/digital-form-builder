...
export class SummaryPageController extends PageController {
  ...
  makeGetRouteHandler() {
    return async (request: HapiRequest, h: HapiResponseToolkit) => {
      ...
      if (model.basePath === "close-contact-form") {
        viewModel.details = this.convertDetailsForCloseContact(
          viewModel.details
        );
      }
  ...
  convertDetailsForCloseContact(details) {
    const detailItem = ({ name, url, value }) => {
      return {
        name:
          name === "Full name" ? "FullName" : name.split(" ")[0] + "Details",
        path: url,
        label: name,
        value: value,
        rawValue: value,
    ...
    const contactDetails = (sectionName) => {
      return details
        .find((item) => item.name === sectionName)
        .items.filter(
          (item) => !/close contact|name/g.test(item.label) && item.value
        )
        .map((item) => item.value)
        .join("\n");
    };

    const closeContactItem = (itemName) =>
      JSON.parse(
